...
#  此处读入图片，作为接口
pic_name = 'picture/testresult.jpg'
...
largest_area = 0
largest_contour_index = -1  # 初始化为-1，以便检查是否找到了轮廓
# 遍历所有轮廓
for i, contour in enumerate(contours):
    area = cv2.contourArea(contour, False)  # 计算每个轮廓的面积
    if area > largest_area:
        largest_area = area
        largest_contour_index = i
# 检查是否找到了轮廓
if largest_contour_index != -1:
    maxContour = contours[largest_contour_index]  # 获取最大轮廓
    # 接下来的代码可以用来绘制轮廓或其他处理
else:
    print("未找到有效轮廓")
